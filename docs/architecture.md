...
| GET | `/notes` | Все заметки профиля списком, фильтр `?book=<id>` (N8) |
...
1. **Кодировка**: UTF-8 (BOM ок) → windows-1251 → с заменой битых символов.
2. **Главы**: строки-заголовки уровня `#`/`##` — границы глав; `###` и глубже остаются заголовками внутри главы. Текст до первого заголовка приклеивается к первой главе.
3. **Без заголовков**: весь файл одной главой; длинный (>12 000 знаков) нарезается по абзацам (`_split_html`, ~4000 знаков).
4. **Рендер**: абзацы по пустым строкам (переносы строк внутри абзаца склеиваются), `**жирный**` → `<strong>`, `*курсив*` → `<em>`, весь текст экранируется — сырой HTML не выполняется; итог проходит общий `_clean_html`.
5. Заголовок книги — имя файла (без расширения), автор — «Неизвестный автор»; пустой файл — `ValueError` → карточка «Пустой файл».
